...
SYSTEM = """
### Instructions ###
You are an expert in AI.Please describe the main points of the paper in bullet points based on your understanding of the content of the paper.\n
Your answer will be given a score. The more accurate your summary, the higher your score. You should try to increase it as much as possible. \n
If you provide information that is not included in the paper, your score will be set to negative infinity.

### Bullet point constraints ###
- Maximum 5
- Answer in Japanese
- Each bullet point is limited to 100 characters in Japanese.

### Contents of the paper to be covered ###
{text}
...
def make_xml_file(pdf_file_name: str, is_debug: bool = False) -> Element:
    ...
    cp = subprocess.run(
        # f"java -Xmx4G -jar /workspace/grobid-0.7.2/grobid-core/build/libs/grobid-core-0.7.2-onejar.jar -gH /workspace/grobid-0.7.2/grobid-home -dIn /workspace/pdf/{pdf_file_name}/ -dOut /workspace/xml -exe processFullText",
        f"java -Xmx4G -jar ./grobid-0.7.2/grobid-core/build/libs/grobid-core-0.7.2-onejar.jar -gH ./grobid-0.7.2/grobid-home -dIn ./pdf/{pdf_file_name}/ -dOut ./xml -exe processFullText",
        shell=True,
    )
    if is_debug:
...
def write_markdown(sections: List[str], pdf_text_list: List[str], pdf_file_name: str) -> str:
    """
    Markdownファイルを作成
    Args:
        sections: セクションのリスト
