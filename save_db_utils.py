...
DATABASE_ID_DICT = {
    "paper": os.environ.get("NOTION_DB_ID"),
}
...
def create_page(paper: arxiv.Result, summary: str, database_id: str, is_debug: bool = False) -> None:
    ...
    if paper.journal_ref is not None:
        PROPERTIES["Journal"]["select"]["name"] = paper.journal_ref
    if paper.doi is not None:
        PROPERTIES["DOI"]["url"] = paper.doi
...
def write_to_notion_page(markdown_text: str, paper: arxiv.Result, page_id: str, is_debug: bool = False) -> None:
    ...
    payload["children"].append(
                    {f"heading_1": {"rich_text": [{"text": {"content": "Abstract"}}]}}
                )
    payload["children"].append(
                    {"paragraph": {"rich_text": [{"text": {"content": paper.summary}}]}}
                )
    for sentence in markdown_text.split("\n"):
        ...
        elif len(sentence) > 0 and  "-" == sentence[0]:
            payload["children"].append(
                    {f"bulleted_list_item": {"rich_text": [{"text": {"content": " ".join(sentence.split(" ")[1:])}}]}}
                )
        else:
